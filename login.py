...
class LoginScreen:
    def __init__(self):
        ...
        self.login_header = customtkinter.CTkLabel(
            self.fields_container, text="Login", text_font=HEADER_FONT, padx=0, pady=0
        )
        self.login_header.place(anchor="center", x=322, y=29)

        self.login_username = customtkinter.CTkLabel(
            self.fields_container, anchor="w", text="Username: ", text_font=SMALLER_FONT
        )
        self.login_username.place(anchor="center", x=205, y=76)

        self.login_password = customtkinter.CTkLabel(
            self.fields_container, anchor="w", text="Password: ", text_font=SMALLER_FONT
        )
        ...
        self.validation_text = customtkinter.CTkLabel(
            self.fields_container, text="", text_font=EVEN_SMALLER_FONT)
    ...
    def register_button(self):
        if self.state == 'register':
            ...
            new_details = [
                (str(self.login_entry.get()), str(self.password_entry.get()))
            ]
